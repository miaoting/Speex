...
- (void)startRecording:(SpeexData *) speexData {
    ...
    if (!self.encapsulator) {
        self.encapsulator = [[Encapsulator alloc] initWithFileName:speexData];
        self.encapsulator.delegete = self;
    }
    else {
        [self.encapsulator resetWithFileName:speexData];
    }
    ...
    self.timerTimeout = [NSTimer scheduledTimerWithTimeInterval:5 target:self selector:@selector(timeoutCheck:) userInfo:nil repeats:NO];
}
...
- (void)encapsulatingOver {
    if (self.delegate) {
        [self.delegate recordingFinished:[self recordedTimeInterval]];
    }
}
